...
/**
 * ABle Markets — PHP API Entry Point
 *
...
$allowedOrigins = [
    'http://localhost:3000',
    'http://localhost:5173',
    $_ENV['FRONTEND_URL'] ?? 'https://ablemarkets.com',
];
...
if ($method === 'POST' && $path === '/auth/login') {
    (new AbleMarkets\Api\Auth\AuthController())->login();

} elseif ($method === 'POST' && $path === '/auth/register') {
    (new AbleMarkets\Api\Auth\AuthController())->register();

} elseif ($method === 'POST' && $path === '/auth/logout') {
    (new AbleMarkets\Api\Auth\AuthController())->logout();

// ── Leads ─────────────────────────────────────────────────────────────────────
} elseif ($method === 'POST' && $path === '/leads') {
    (new AbleMarkets\Api\Leads\LeadsController())->store();

// ── Admin Dashboard ───────────────────────────────────────────────────────────
} elseif ($method === 'GET' && $path === '/admin/dashboard/stats') {
    (new AbleMarkets\Api\Admin\DashboardController())->stats();

// ── Admin Users ───────────────────────────────────────────────────────────────
} elseif ($method === 'GET' && $path === '/admin/users') {
    (new AbleMarkets\Api\Admin\UsersController())->index();

} elseif ($method === 'GET' && matchRoute('/admin/users/{id}', $path, $params)) {
    (new AbleMarkets\Api\Admin\UsersController())->show($params['id']);

} elseif ($method === 'PUT' && matchRoute('/admin/users/{id}', $path, $params)) {
    (new AbleMarkets\Api\Admin\UsersController())->update($params['id']);

} elseif ($method === 'PATCH' && matchRoute('/admin/users/{id}/status', $path, $params)) {
    (new AbleMarkets\Api\Admin\UsersController())->updateStatus($params['id']);

} elseif ($method === 'DELETE' && matchRoute('/admin/users/{id}', $path, $params)) {
    (new AbleMarkets\Api\Admin\UsersController())->destroy($params['id']);

// ── Admin Transactions ────────────────────────────────────────────────────────
} elseif ($method === 'GET' && $path === '/admin/transactions') {
    (new AbleMarkets\Api\Admin\TransactionsController())->index();

} elseif ($method === 'PATCH' && matchRoute('/admin/transactions/{id}/approve', $path, $params)) {
    (new AbleMarkets\Api\Admin\TransactionsController())->approve($params['id']);

} elseif ($method === 'PATCH' && matchRoute('/admin/transactions/{id}/reject', $path, $params)) {
    (new AbleMarkets\Api\Admin\TransactionsController())->reject($params['id']);
