...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // authRegister maps form → PHP snake_case payload via context
      ...
      <div className="min-h-screen bg-surface flex items-center justify-center p-8">
        <div className="bg-white border border-surface-border rounded-xl p-12 max-w-md w-full text-center shadow-card">
          ...
          <p className="text-[#4A5568] leading-relaxed mb-8">
            Your Vantage Markets account has been created. Please check your
            email to verify your address before logging in.
          </p>
    ...
    <div className="min-h-screen bg-primary flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[420px] flex-shrink-0 border-r border-white/10 p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <div className="space-y-4">
            {[
              ...
              "$50 minimum deposit to get started",
              "Demo account available instantly",
              "Regulated by ASIC, FCA, CIMA, VFSC",
            ].map((item) => (
        ...
        <p className="text-xs text-white/25 relative z-10">
          CFDs involve significant risk. You could lose more than your initial investment.
        </p>
      </div>
          ...
          <div className="flex items-center gap-3 mb-8">
            {[1, 2].map((s) => (
              <div key={s} className="flex items-center gap-3">
                ...
                {s < 2 && (
                  <div className={`h-0.5 w-12 rounded ${step > s ? "bg-accent" : "bg-surface-border"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="bg-white border border-surface-border rounded-xl p-10 shadow-card">
            ...
            {step === 1 && (
              <div className="space-y-5">
                ...
                <Input label="Phone Number (optional)" type="tel" placeholder="+234 800 000 0000"
                  value={form.phone}
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {[
                      { val: "standard", label: "Standard",  sub: "0 commission" },
                      { val: "raw",      label: "Raw ECN",   sub: "0.0 pips" },
                      { val: "demo",     label: "Demo",      sub: "Virtual funds" },
                    ].map((t) => (
                      <label key={t.val}
                        className={`border-2 rounded-[10px] p-3 cursor-pointer text-center transition-all ${form.accountType === t.val ? "border-accent bg-accent/5" : "border-surface-border hover:border-accent/40"}`}>
                        <input type="radio" name="accountType" value={t.val}
                          checked={form.accountType === t.val}
                          onChange={() => update("accountType", t.val)}
                          className="sr-only" />
                ...
                <button onClick={handleNext}
                  className="w-full py-3.5 rounded-[12px] bg-accent text-white font-bold text-sm hover:bg-accent-light transition-all hover:-translate-y-0.5">
                  Continue
                </button>
              </div>
            )}
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    <div key={field}>
                      <label className="flex items-start gap-3 cursor-pointer">
                        <input type="checkbox" checked={form[field]}
                          ...
                          className="accent-accent mt-0.5 w-4 h-4 flex-shrink-0" />
                ...
                <div className="flex gap-3 pt-2">
                  <button type="button" onClick={() => setStep(1)}
                    className="flex-1 py-3.5 rounded-[12px] bg-surface border border-surface-border text-primary font-semibold text-sm hover:border-accent hover:text-accent transition-all">
                    Back
                  </button>
